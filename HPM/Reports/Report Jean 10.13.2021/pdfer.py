...
def unite_pictures_into_pdf(
    ...
    if picturesAreInRootFolder == False:
        foldersInsideFolderWithPictures = sorted_nicely(
            glob.glob(pathToPictures + "\\*\\")
        )
                    ...
                    if (numberOfFoldersAdded % numberOfEntitiesInOnePdf) == 0:
                        ...
                        if numberOfEntitiesInOnePdf > 1:
                            filename = os.path.join(
                                ...
                                + nameOfPart
                                + "_"
                                + str(numberOfFoldersAdded + 1)
                                + "-"
                                + str(endNumber)
                                + "_of_"
                                + str(len(picturesPathsForEachFolder))
                                + ".pdf",
                            )
                        ...
                        if (numberOfPicturesAdded % numberOfEntitiesInOnePdf) == 0:
                            ...
                            if numberOfEntitiesInOnePdf > 1:
                                filename = os.path.join(
                                    ...
                                    + nameOfPart
                                    + "_"
                                    + str(numberOfPicturesAdded + 1)
                                    + "-"
                                    + str(endNumber)
                                    + "_of_"
                                    + str(totalNumberOfPictures)
                                    + ".pdf",
                                )
                            elif numberOfEntitiesInOnePdf == 1:
                                filename = os.path.join(
    ...
    if picturesAreInRootFolder == True:
        ...
        for iExtension in listWithImagesExtensions:
            picturesInsideFolderWithPictures.extend(
                glob.glob(pathToPictures + "\\*." + iExtension)
            )
        picturesInsideFolderWithPictures = sorted_nicely(
            picturesInsideFolderWithPictures
        )
